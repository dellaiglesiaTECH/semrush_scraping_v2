...
screenshots_folder='cookies_screenshots_2024'
if not os.path.isdir(screenshots_folder):
    os.makedirs(screenshots_folder)

if not os.path.exists("HTML_2"):
    os.makedirs("HTML_2")
...
#for file in semrush_files:
    ...
    #if len(df_temp['id']=='nbe.com.eg')>0:
   #     print('this %s contains nbe' %file)
  #  df=df.append(df_temp)
 #   df=df.reset_index(drop=True)
#unique_websites=list(set(list(df['id'])))
#i=0

for file in semrush_files:
    df_temp = pd.read_csv(file)
 #   if 'id' in df_temp.columns and df_temp['id'].eq('nbe.com.eg').any():
 #       print(f'this {file} contains nbe')
...
##                continue
    ...
    with open(os.path.join("HTML_2", filename), "w", encoding="utf-8") as file1:
        file1.write(html)
##try:
##    driver.get('http://unimib.it')
##except:
##    print('error in page')
##    continue
